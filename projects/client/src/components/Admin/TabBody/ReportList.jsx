...
export const ReportList = () => {
  ...
  const getJournals = useCallback(async () => {
    try {
      const journalURL = url + `all_journal?sort=${sort}&direction=${direction}&pagination=${pagination}`;
              ...
              return (
                <Tbody key={index} bg={"#efdbef"} _hover={{ bg: "#e9cde8" }}>
                  <Tr>
                    ...
                    <Td textAlign={"center"}>{item.StockMutationId ? item.StockMutationId : "-"}</Td>
                    <Td textAlign={"center"}>{item.TransactionId ? item.TransactionId : "-"}</Td>
                    <Td textAlign={"center"}>
                      <Stack>
